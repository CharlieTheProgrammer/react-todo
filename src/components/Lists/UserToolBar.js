	...
	return (
		<>
			<section className="d-flex my-2 justify-content-between align-items-center">
				...
				<div className="lists-nav-searchbar w-100 px-2">
					<input
						...
						onChange={evt => props.updateSearchTerm(evt.target.value)}
			...
			<section className=" d-flex align-items-center mt-1 ml-1 mb-2">
				...
				<p className="lists-nav-displayname m-0 pl-2 pointerOnHover" onClick={() => props.toggleListsDrawer()}>
					{displayName}
				</p>
			</section>
			<ListsDrawer showListsDrawer={props.showListsDrawer} logOut={props.logOut} />
		</>
	)
}
